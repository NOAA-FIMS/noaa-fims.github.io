...
[R/setup_default_parameters.R]{.underline} This bit of code contains several functions necessary for broadly and specifically specifying default parameters in FIMS. The nested functions are as follows:
...
```{r}
#| eval: false
setup_default_AgeSpecific <- function(data) {
...
Once the functionality and default parameters were set, it was time to add a vignette that demonstrates the selectivity-at-age function working in action. We saved a copy of the existing [vignettes/fims-demo.Rmd]{.underline} as [fims-demo-age-spec-sel.Rmd]{.underline} and when setting up the default parameters, also did the following:
...
Finally, we added a part in the code that creates "parameters_4_model" to specify which parameters are estimated and where we want the estimation to start:
...
```cpp
gtest_discover_tests(AgeSpecificSelectivity_Evaluate)
```
...
[tests/testthat/test-setup_default_AgeSpecific.R]{.underline} - This code specifically tests that the setup_default_AgeSpecific() function from R/setup_default_parameters.R is working  

[tests/testthat/test-setup_default_Selectivity.R]{.underline} - This code specifically tests that the setup_default_Selectivity() function from R/setup_default_parameters.R is working  
...
Once all that was said and done, we had one final contribution to make to the FIMS code base. We updated the manual! In [man/setup_default_Selectivity.Rd]{.underline} we updated the listed "allowable forms" of selectivity to include AgeSpecific. We were legit - in the FIMS manual and everything!
