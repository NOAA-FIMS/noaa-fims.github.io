...
function initQuartoAccessibilityFixes() {
  ...
  // covering the cards, and the search/filter inputs lack accessible names.
  const initListingFixer = () => {
    const listing = document.getElementById("listing-listing");
    // If the listing container isn't on the page, do nothing and signal to retry.
    if (!listing) {
      return false;
    }

    // This function contains the core logic to fix labels.
    const applyListingLabels = () => {
      // Add accessible names to the filter and sort controls.
      ...
      listing.querySelectorAll("a.no-external[href]").forEach((link) => {
        // If the link already has text or a label, skip it.
        if (link.textContent.trim() || link.getAttribute("aria-label")) {
          return;
        }

        // Find the parent card and then find the title within it.
        ...
        if (title) {
          ...
          // Fallback to guarantee the Axe error is cleared even if a title can't be found.
          link.setAttribute("aria-label", "Read more about this item");
          link.setAttribute("title", "Read more about this item");
        }
      });
    };

    // Run the fix once on initial load.
    applyListingLabels();

    // Set up a MutationObserver to re-apply fixes whenever Quarto's List.js
    // engine rebuilds the listing HTML (e.g., after filtering).
    const observer = new MutationObserver(() => {
    ...
    return true; // Signal that initialization was successful.
  };

  // Poll for the listing element to appear. This is more robust than a simple
  // DOMContentLoaded listener for content generated by third-party scripts.
  const maxRetries = 10;
  let retries = 0;
  const intervalId = setInterval(() => {
    // Once initListingFixer succeeds (returns true) or we run out of retries, stop polling.
    if (initListingFixer() || ++retries > maxRetries) {
